...
class IndexController extends Controller {
    public function displayIndex($where, $action_name, $title, $display_page) {
        ...
        $check_in = M("check_in")->where(array("user_id" => $where['img_user'], "check_in_time" => array(array('egt', strtotime(date("Y-m-d 0:0:0"))), array('elt', strtotime(date("Y-m-d 23:59:59"))), 'and')))->find();
        $this->assign("file_list", $file_list);
        $this->assign("page", $page->show());
        $this->assign("action_name", $action_name);
        $this->assign("title", $title);
        $this->assign("check_in", ($check_in) ? 1 : 0);
